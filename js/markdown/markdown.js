...
(function(root, factory) {
    ...
    function slidify(markdown, options) {
        ...
        for (var i = 0, len = sectionStack.length; i < len; i++) {
            ...
            if (sectionStack[i] instanceof Array) {
                /*markdownSections += '<slides class="slides"><slide class="slide"><section class="slide-wrapper" ' + options.attributes + '>';

				sectionStack[i].forEach(function(child) {
					markdownSections += '<slide class="slide"><section class="slide-wrapper" data-markdown>' + createMarkdownSlide(child, options) + '</section></slide>';
				});

				markdownSections += '</section></slide></slides>';*/
                sectionStack[i].forEach(function(child) {
                    markdownSections +=
                        '<slide class="slide"><section class="slide-wrapper" data-markdown>' +
                        createMarkdownSlide(child, options) +
                        "</section></slide>";
                });
            } else {
                markdownSections +=
                    '<slide class="slide"><section class="slide-wrapper" ' +
                    options.attributes +
                    " data-markdown>" +
    ...
    function convertSlides() {
        ...
        var DEFAULT_SLIDE_IMG_CONTENT =
            sections[0].getAttribute("data-img-content", true) || "markdown";
        ...
        for (var i = 0, len = sections.length; i < len; i++) {
            ...
            if (!section.getAttribute("data-markdown-parsed")) {
                ...
                section.innerHTML = html.replace(
                    RegExp(/<img src=".\//gi),
                    '<img src="./' + DEFAULT_SLIDE_IMG_CONTENT + "/"
                );
                addAttributes(
                    section.parentNode,
                    section,
        ...
        $(".slide.fill").append(
            '<div style="background-color:rgba(55,68,84,.82);position:absolute;left:0;right:0;top:0;bottom:0;width:100%;height:100%;z-index:-1;">'
        );
    }
